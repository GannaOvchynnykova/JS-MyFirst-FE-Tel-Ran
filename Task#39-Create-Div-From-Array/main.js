...
function createDiv(item) {
    ...
    for (let i = 0; i < item.length; i++) {
        ...
        //divBoxMusic.classList.add('divBoxMusic', itemClass)
        ...
        divBoxMusic.innerHTML = `<img
            ...
            <h5>${item[i].title}</h5>
            <h5>${item[i].author}</h5>`;
        divBoxMusic.style.backgroundColor = `rgb(${getColor()})`;
        content.appendChild(divBoxMusic);
    }
}

/*
const musicCollection = [jazz, rock, blues];
const buttons = document.querySelectorAll('button');

buttons.forEach(btn => btn.onclick = (event) => {
    createDiv(event.target.id);
});*/
...
jazzBtn.onclick = (event) => {
    createDiv(jazz, event.target.innerHTML);
}

rockBtn.onclick = (event) => {
    createDiv(rock, event.target.innerHTML);
}

bluesBtn.onclick = (event) => {
    createDiv(blues, event.target.innerHTML);
}
